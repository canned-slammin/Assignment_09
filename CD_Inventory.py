...
# SBurner, 2020-Dec-11, Built out code TODOs per Assignment 09
#------------------------------------------#
    ...
    if strChoice == 'l':
        ...
        while True:
            ...
            if strCDChoice == 'a':
                ...
                PC.DataProcessor.add_track(IO.ScreenIO.get_track_info(), cd)
                IO.ScreenIO.show_tracks(cd)
                continue
            if strCDChoice == 'd':
                IO.ScreenIO.show_tracks(cd)
                continue
            if strCDChoice == 'r':
                print('Enter position of track to delete:')
                PC.DataProcessor.del_track(IO.ScreenIO.get_track_id(), cd)
                IO.ScreenIO.show_tracks(cd)
                continue
            if strCDChoice == 'x':
                break
